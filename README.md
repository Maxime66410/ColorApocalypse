...
All three loaders build from the same shared codebase (`src/`), with a small loader-specific bootstrap package each (`org.furranystudio.colorapocalypse.forge` / `.fabric` / `.neoforge`). Everything else, block/mob/item color detection, destruction logic, the roulette timing, settings, is identical across all of them.

## Building

`forge/`, `fabric/` and `neoforge/` are each their own standalone Gradle project (no root `build.gradle`/`settings.gradle`), build them with `-p`:
...
Requires Gradle 9.7+ and a JDK 17+ to run Gradle itself (the mod targets Java 25). The Fabric and NeoForge builds specifically need Gradle running on a JDK 25.